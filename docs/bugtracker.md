...
## B-009: Нормали чанков без heightAmp/R — градиент рельефа в 800× сильнее нормы

**Дата:** 2026-05-08
**Компонент:** `src/planet/LODPlanet.ts:generateChunk()`
**Severity:** critical (чёрные пятна на дневной стороне, освещённая ночная сторона, размытый терминатор)
**Статус:** resolved

**Ключевые слова:** `normal`, `gradient scale`, `heightAmp`, `planet radius`, `normal perturbation`, `height field`, `PBR shading`, `day/night terminator`

### Симптомы

На планете одновременно наблюдаются два визуальных дефекта:

1. **Чёрные «рваные» пятна на дневной стороне** — участки планеты полностью чёрные (0, 0, 0) в прямом солнечном свете.
2. **Зелёные области биомов на ночной стороне** — неосвещённая часть планеты показывает цвета растительности.
3. **Терминатор отсутствует** — граница дня и ночи размыта, нет резкого перехода.
4. **«Лучи подсвечивают сферу изнутри»** — неосвещённые участки выглядят так, будто свет идёт изнутри планеты наружу.
5. При движении камеры к ночной стороне чёрные области растут.

Дефект проявляется после удаления HemisphericLight (B-007). Единственный источник света — DirectionalLight.

### Root cause

В `generateChunk()` нормаль к поверхности со смещением высоты вычисляется как:

```typescript
const nx = dir.x - dhdu * dirU.x - dhdv * dirV.x;
```

Поверхность планеты задана как `P = D * (R + h * heightAmp)`, где:
- R = 6371 — радиус планеты
- heightAmp = 8 — амплитуда высот рельефа
- h ∈ [0, 1] — нормализованная высота

Касательный вектор: `∂P/∂u = D * heightAmp * ∂h/∂u + (R + h*heightAmp) * ∂D/∂u`.
Первое слагаемое — вклад рельефа (~8), второе — вклад сферы (~6371).

**Правильная нормаль после нормализации:**
`N ≈ D - (heightAmp / R) * (∂h/∂u * dirU + ∂h/∂v * dirV)`

heightAmp/R = 8/6371 ≈ 0.00126. Этот коэффициент масштабирует градиент высоты
относительно радиуса планеты. **Без него** градиент рельефа в ~800× сильнее нормы:
- На крутых склонах (dhdu ≈ 2): возмущение ≈ 2·dirU — сравнимо с радиальным направлением D.
  Нормаль уходит на 30–60°, на очень крутых склонах — **внутрь планеты**.
- Дневная сторона: N·L < 0 (нормаль внутрь, свет снаружи) → чёрные пятна
- Ночная сторона: N·L > 0 (нормаль внутрь, «смотрит» на солнце сквозь планету) → зелёные области
- Терминатор размыт: хаотичные направления нормалей дают случайные N·L ≈ 0

### Fix

Добавлен коэффициент `gs = heightAmp / R`:
...
С ним на равнинах N ≈ D (радиальная нормаль), на склонах возмущение ≈ 0.00126·dirU
(отклонение ~0.07°) — физически корректно для рельефа 8м на сфере 6371м.

### Почему не было заметно раньше

HemisphericLight (intensity 0.3) сглаживал проблему: его half-lambert освещение
создавало фоновую засветку даже при N·L < 0, маскируя чёрные пятна.
После удаления HemisphericLight (B-007) DirectionalLight стал единственным
источником — дефект стал полностью виден.

### Верификация

- `npm run dev`: чёткий терминатор, ночная сторона чёрная, дневная — равномерно освещена
- Зелёные биомы видны только на освещённой стороне
- При пересечении терминатора поверхность плавно темнеет (правильный диффузный переход)

### Применённые фиксы

| Файл | Строка | Изменение |
|------|--------|-----------|
| `src/planet/LODPlanet.ts` | ~276 | Добавлен `const gs = heightAmp / R`, градиенты умножены на `gs` |
