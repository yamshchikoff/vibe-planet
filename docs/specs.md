...
- **Yaw (A/D)** — вращение вокруг **мировой** оси Y (left-multiply: `q = q_yaw * q`).  
  Всегда разворачивает самолёт в горизонтальной плоскости, не вызывая крен.  
  Это соответствует работе руля направления в скоординированном развороте.
- **Pitch (W/S)** — вращение вокруг **локальной** оси X (right-multiply: `q = q * q_pitch`).  
  При крене 90° питч ведёт к развороту в горизонтальной плоскости.
- **Roll (Q/E)** — вращение вокруг **локальной** оси Z (right-multiply: `q = q * q_roll`).
