...
Ожидаемое поведение на крейсерской скорости (8 km/с, горизонт):

| Режим | Net ускорение | Эффект |
|-------|--------------|--------|
| Полный газ (Shift) | ~8.7 km/с² | скорость удваивается за ~1 с |
| Ноль газа (throttle=0) | ~-1.3 km/с² | скорость падает на ~1.3 km/с каждую секунду |
| Равновесие (drag = thrust) | 0 | v_terminal = sqrt(MAX_THRUST / DRAG_COEFF) ≈ 22.4 km/с |

Throttle 0 → скорость падает до ~4 km/с за ~5 с (ощутимое замедление).
Throttle 1 → скорость растёт до ~16 km/с за ~1 с (резкий разгон).
