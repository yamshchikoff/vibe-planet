...
Метод:
1. Вычислить четыре угла чанка в мировых координатах: преобразовать
   (u, v) углов сетки через `uvToDir(face, u, v)`, умножить на радиус планеты
2. Измерить длины четырёх рёбер чанка в мировых единицах.
   `edgeWorld = max(длины четырёх рёбер)` — консервативная оценка
   (используется максимальное ребро для предотвращения недооценки
   screen-space размера)
3. Спроецировать на экран: `screenPx = (edgeWorld / distance) ×
   (viewportHeight / (2 × tan(fovY / 2)))`

В отличие от приближения `(π × R) / (2^depth)`, которое даёт погрешность
до ~30% для угловых чанков кубической сферы (низкие глубины, края граней),
точное вычисление через uvToDir учитывает реальное искажение проекции.
Для производительности — кэшировать `edgeWorld` на каждый (face, depth),
так как в первом приближении оно не зависит от tx,ty.
